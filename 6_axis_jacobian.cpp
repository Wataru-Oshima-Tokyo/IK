...
Eigen::Matrix<double, 6, 6> jacobian_matrix(const Eigen::VectorXd& joint_angles, const Eigen::VectorXd& link_lengths){

    // 6軸用に変更したjacobian_matrix関数をここに挿入
    ...
    double l5 = link_lengths(4);
    double l6 = link_lengths(5);
    ...
    Eigen::Matrix3d R3_4 = rotation_matrix_x(theta4);
    Eigen::Matrix3d R4_5 = rotation_matrix_x(theta5);
    Eigen::Matrix3d R5_6 = rotation_matrix_x(theta6);
...
Eigen::VectorXd inverse_kinematics(const Eigen::Vector3d& target_position, const Eigen::VectorXd& link_lengths, const Eigen::VectorXd& initial_joint_angles) {
    ...
    while (iteration < MAX_ITERATIONS) {
        Eigen::Matrix<double, 6, 6> J = jacobian_matrix(joint_angles, link_lengths);
        Eigen::Matrix<double, 6, 6> J_pseudo_inverse = J.completeOrthogonalDecomposition().pseudoInverse();

        Eigen::Vector3d current_position = J.block<3, 1>(0, 5);
        Eigen::Vector3d position_error = target_position - current_position;
        Eigen::VectorXd joint_angle_update = J_pseudo_inverse.block<6, 3>(0, 0) * position_error;
...
int main() {
    ...
    link_lengths << 2, 3, 3, 1, 1, 1;
    Eigen::VectorXd initial_joint_angles(6);
    initial_joint_angles << M_PI / 4, M_PI / 4, M_PI / 4, M_PI / 4, M_PI / 4, M_PI / 4;
